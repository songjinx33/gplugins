...
class LumericalEmeSimulation(Simulation):
    ...
    def __init__(
        ...
        **settings,
        ...
        """
        Set up EME simulation based on component geometry and simulation settings.
        Optionally, run convergence sweeps.
        ...
        Parameters:
            ...
            simulation_settings: EME simulation settings
            convergence_settings: EME convergence settings
            dirpath: Root directory where simulations are saved. A sub-directory
                labeled with the class name and hash is be where simulation files
                are saved.
            hide: Hide simulation if True, else show GUI
            run_mesh_convergence: If True, run sweep of mesh and monitor sparam convergence.
            run_cell_convergence: If True, run sweep of number of cells in central
                group span and monitor sparam convergence.
            run_mode_convergence: If True, run sweep of number of modes and monitor
                sparam convergence.
            run_overall_convergence: If True, run combination of mesh and cell
                convergence to quicken convergence.
        ...
        # Run convergence testing if no convergence results are available or
        # user wants to override convergence results or if setup has changed
        if (
    ...
    def update_mesh_convergence(self, plot: bool = False) -> pd.DataFrame:
        """
        Update simulation based on mesh convergence testing. Updates both
        Lumerical session and simulation settings.
    ...
    def update_cell_convergence(self, plot: bool = False) -> pd.DataFrame:
        """
        Update simulation based on cell convergence testing (number of slices
        across the device center). Updates both Lumerical session and simulation
        settings.
    ...
    def update_mode_convergence(self, plot: bool = False) -> pd.DataFrame:
        """
        Update simulation based on mode convergence testing (number of modes
        required to be accurate). Updates both Lumerical session and simulation
        settings.
    ...
    def update_overall_convergence(self, plot: bool = False) -> pd.DataFrame:
        """
        Runs mesh and cell convergence simultaneously, switching strategies
        every time sparam variation increases.
    ...
    def get_mode_coupling(
        ...
        Parameters:
            ...
            max_coupled_mode: Maximum mode number to consider; all modes below
                this mode will be considered as well.
    ...
    def plot_mode_coupling(
        ...
        """
        Plot mode coupling coefficients from the input_mode to the
        max_coupled_mode vs. position. Mode numbers start from 1 (fundamental mode).
