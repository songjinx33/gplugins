...
from gplugins.lumerical.simulation_settings_gpu import (
...
class LumericalFdtdSimulation_port(Simulation):
    """
    ...
    def __init__(
        ...
        specified_filepath : PathType | None = "",
        hide: bool = False,
...
#        self.filepath_fsp = filepath_fsp = filepath_dat.with_suffix(".fsp")
        if not specified_filepath :  # specified_filepath  is empty or None
            self.filepath_fsp = filepath_fsp = filepath_dat.with_suffix(".fsp")
            logger.info(f"{str(self.filepath_fsp)}")
        else:  # specified_filepath  is provided
            specified_filepath_path = Path(specified_filepath )
            self.filepath_fsp = filepath_fsp = specified_filepath_path.with_suffix(".fsp")
        ...
        for layer_name in layer_stack.to_dict():
            ...
            try:
                ...
                if material_name:  # Check if material_name is not empty or None
                    logger.info(f"Pattern material retrieved for {layer_name}: {material_name}")
                    ...
                    # logger.warning(f"No pattern material found for {layer_name}, skipping.")            
                    material_name = s.getlayer(layer_name, "background material")
                    if material_name:  # Check if material_name is not empty or None
                        logger.info(f"Bkg material retrieved for {layer_name}: {material_name}")
                        s.setmaterial(material_name, "wavelength min", ss.wavelength_start * um)
                        s.setmaterial(material_name, "wavelength max", ss.wavelength_stop * um)
                        s.setmaterial(material_name, "tolerance", ss.material_fit_tolerance)
                    else:
                        # logger.warning(f"No background material found for {layer_name}, skipping.")
                        pass
            except:
                logger.warning(
        ...
        if add_mesh_setting:
            s.addmesh()
            mesh_object = f'mesh'
            s.setnamed(mesh_object,"x min",fdtd_xmin)
            s.setnamed(mesh_object,"x max",fdtd_xmax)
            s.setnamed(mesh_object,"y min",fdtd_ymin)
            s.setnamed(mesh_object,"y max",fdtd_ymax)
            s.setnamed(mesh_object,"z",-0.005e-6)
            s.setnamed(mesh_object,"z span",0.020e-6)
            self.add_mesh_region()
        ...
        s.setnamed("FDTD","express mode", ss.express_mode)  # Enable Express Mode  
        # Save simulation
    ...
    def add_mesh_region(
        ...
        dy: int = 0.02,
        dz: float = 0.004,
        mesh_object: str = f'mesh'
    ) -> pd.DataFrame:
        s = self.session
        s.select("mesh")
        s.setnamed("mesh",'override x mesh',0)
        s.setnamed(mesh_object,"dx",dx*1e-6)
        s.setnamed("mesh",'override y mesh',0)
        s.setnamed(mesh_object,"dy",dy*1e-6)
        s.setnamed("mesh",'override z mesh',1)
        s.setnamed(mesh_object,"dz",dz*1e-6)
    ...
    def write_runfile(
        ...
        """
        Run simulation file instead of s parameter sweep; write s-parameters to npz, csv, dat files; or read s-parameters from csv.
        ...
        filepath_sim_settings = filepath.with_suffix(".yml")
        filepath_csv = self.filepath_npz.with_suffix(".csv")
        fspdir = filepath.parent / f"{filepath.stem}_s-parameter"

        if filepath_csv.exists() and not overwrite:
            logger.info(f"Reading T from {filepath_csv!r}")
            sparam_data = pd.read_csv(filepath_csv, index_col=0)
        ...
        for port in ports:
            a_result = s.getresult(f"FDTD::ports::{port}", "S")
            t_result = s.getresult(f"FDTD::ports::{port}", "T")
            
            # Save S and T results with custom keys
            sp[f"s_{port}"] = a_result["S"]  # Save "S" data as "s_port"
            sp[f"t_{port}"] = t_result["T"]  # Save "T" data as "t_port"
                        
        wavelength = a_result["lambda"]/ um
        sp[f"wavelengths"] = wavelength  
        np.savez_compressed(self.filepath_npz, **sp)
        logger.info(f"Writing Sparameters to {self.filepath_npz.absolute()!r}")
        ...
        sparams = {k: sp[k] for k in sp}  # Copy everything from sp
        sparams["wavelength"] = sparams.pop("wavelengths")
        sparam_data = pd.DataFrame(sparams)
        sparam_data.to_csv(filepath_csv)
        logger.info(f"Writing Sparameters to {filepath_csv!r}")
...
class LumericalFdtdSimulation_source(Simulation):
    """
    Lumerical FDTD simulation
        ...
        **settings,
        ...
        for i, port in enumerate(ports):
            ...
            if i == input_port:
                ...
                p0 = f"source"
                s.set("name",p0)
                s.setnamed(p0, "z", z * um)
                s.setnamed(p0, "z span", zspan * um)
                s.setnamed(p0,"override global source settings",0);

                s.addpower()
                p1 = f"input"
                s.set("name",p1)
                s.setnamed(p1, "monitor type",  "2D Y-normal")
                s.setnamed(p1, "z", z * um)
                s.setnamed(p1, "z span", zspan * um)

            else:
                logger.info(f"start add monitor")
                s.addpower()
                p0 = f"transimission"
                s.set("name",p0)
                s.setnamed(p0, "monitor type",  "2D Y-normal")
                s.setnamed(p0, "z", z * um)
                s.setnamed(p0, "z span", zspan * um)
                s.addpower()
                p1 = f"transimission1"
            ...
            deg = port.orientation
            for i_p, p in enumerate([p0,p1]):
                logger.info(f"print{i}  {i_p} {p}")
                if  i == input_port and i_p ==0:
                    if -45 <= deg <= 45:
                        direction = "Backward"
                        injection_axis = "x-axis"
                        s.setnamed(p, "x", (port.x + ss.port_translation) * um)
...
#                    s.setnamed(p, "name", port.name)
                    ...
                    # s.setnamed(p, "direction", direction)
                    s.setnamed(p, "monitor type", injection_axis)
                    if dyp !=0:
                        s.setnamed(p, "y span", dyp * um)
                    if dxp !=0:
                        s.setnamed(p, "x span", dxp * um)
